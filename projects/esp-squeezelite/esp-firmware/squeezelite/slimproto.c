	...
	case 's':
		{
			...
			if (ip == LOCAL_PLAYER_IP && port == LOCAL_PLAYER_PORT) {
				...
				LOG_ERROR("Local streaming not implemented!");
				autostart -= 2;
			} else {
...
static void slimproto_run() {
	...
	while (running && !new_server) {
		...
		while(ev == EVENT_WAITING) {
			if(xTaskCheckForTimeOut(&xTimeOut, &xTicksToWait) != pdFALSE ) {
				ev = EVENT_TIMEOUT;
				break;
			}
			if(recv(sock, buffer + got, 0, MSG_PEEK | MSG_DONTWAIT) > 0) {
				ev = EVENT_READ;
			}
...
static void slimproto_task(void *pvParameters)
{
